...
| `TUSHARE_KLINE_ADJUST` | Tushare A-share daily K-line adjustment mode: `qfq` (default; forward-adjusts `daily` prices via `adj_factor / latest adj_factor` so ex-dividend gaps do not distort moving averages/returns) or `none` (Tushare raw unadjusted prices). | `qfq` | Optional |
...
> - Tushare A-share daily K-lines default to `TUSHARE_KLINE_ADJUST=qfq`: `daily` rows are forward-adjusted by `adj_factor / latest adj_factor`, matching the qfq baseline used by AkShare/Tencent/Baostock/efinance/hithink fallback sources; `pct_chg`/`pre_close` keep Tushare's original ex-dividend basis. If `adj_factor` fetching fails or has missing trade dates, the fetcher degrades to unadjusted prices and logs a warning without affecting availability.
