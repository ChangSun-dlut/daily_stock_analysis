...
- [修复] 选股 LLM 重排失败回退本地评分（提示「LLM 重排失败：fell back to screen_score」+ `coverage below threshold: 0.00; errors=['empty_response']`）：根因是主模型 `anthropic/MiniMax-M2.7` 属推理型模型，输出预算被 thinking token 占满——实测 15 只候选的重排请求 `max_tokens=2048` 时 `completion_tokens=1677 / reasoning_tokens=1577`，正文仅剩约 100 token，prompt 稍大即 `finish_reason=length` 且正文为空。litellm 对该模型不支持 `thinking={"type": "disabled"}`（`UnsupportedParamsError`），故改为放大输出预算：新增 `scripts/patch_alphasift_ranker.py`（幂等，给安装包的 `alphasift.ranker._call_llm` 打补丁），把重排 `max_tokens` 下限抬到 8192（`max_tokens` 只是上限、不增加实际消耗），并在 `finish_reason=length`/空正文时按 3 倍递增重试（最多 3 次、上限 16384）；同时把重排请求超时下限提到 150s（预算抬高后生成耗时变长，60s 会超时）。修复后实测 `ranked=True, coverage=1.00, errors=[]`。运行时 pipeline 走的是 pip 安装的 `alphasift` 包而非仓库派生副本，故补丁作用于 site-packages，并在 `main.py` / `server.py` 启动检查中告警（与既有 `patch_alphasift_daily.py` 同款机制）。
- [新功能] 大盘复盘「明日交易计划」升级为「明日作战地图」格式：在原有带标签结论（结论/仓位区间/关注方向/回避方向/触发失效条件，保持 share_image 海报字段解析兼容）之后，要求 LLM 输出 Markdown 表格（表头固定为 时间/触发｜观察点｜应对动作，必含 6 行：竞价、量能（缩量/平量/放量三情形阈值与应对）、关键点位（突破/跌破加减仓）、盘中情绪监控（涨停/跌停家数、炸板率等可量化信号）、重要事件、仓位纪律），表格后以「核心矛盾一句话：」点明多空主要矛盾；中英文模板（完整/降级共 4 个分支）同步生效。Web 端 `ReportMarkdownBody`（remark-gfm）与分享图 `markdown2 tables` extra 均原生支持表格渲染，无需改渲染层。
- [新功能] 新增「板块流动分析」卡片（选股页板块资金流向与选择策略之间）：基于板块资金流向（含盘中实时兜底）、板块轮动与全天涨停时间线生成 AI 复盘——「全天流动总结」+「板块带头标的·涨停时点记录」（东财涨停池 `stock_zt_pool_em` 按板块聚合，取连板数最高/首封最早者为带头标的，记录首次封板时间、连板数、炸板次数）+ 4 阶段分时时间线（竞价 09:15~09:30 / 上午 09:30~11:30 / 午后 13:00~14:30 / 尾盘 14:30~15:00，每阶段 LLM 叙述 + 涨停事件按首次封板时间排序）+「后市关注」；LLM 走项目统一入口 `GeminiAnalyzer.generate_text`（channel Router + fallback 链，裸 litellm.completion 会因环境缺 ANTHROPIC/OPENAI_API_KEY 而 401），要求只输出 JSON 并用稳健解析器提取（容忍 ```json 围栏与字符串内花括号）；LLM 输出经 `_normalize_sector_flow_phases` 规范化到固定 4 阶段，缺失事件由涨停时间线数据兜底回填；LLM 不可用时整卡退规则兜底，保证功能始终有内容；结果缓存 30 分钟，卡片提供「强制刷新」按钮并标注来源（AI 复盘 / 规则汇总）。新增 `tests/test_sector_flow_analysis.py`（17 例）。
- [新功能] 新增「老鸭头」选股策略（`old_duck_head`）：按「鸭颈放量上穿 MA60 → 鸭头缩量回调洗盘 → 鸭嘴放量突破前高」的经典形态量化选股（阈值取自通达信/同花顺选股公式与雪球/东财/百家号实战帖的多源共识）。新增 7 个日线形态特征列——`barslast_ma5_cross_ma60`（鸭颈成熟度）、`duck_nose_gap_pct`（鸭鼻孔贴合度）、`death_then_golden_5_10`（死叉后再金叉）、`duck_beak_volume_contraction`（量芝麻点，多源共识为回调期量能峰/谷 ≥ 2）、`ma60_slope_20d_pct`（60 线斜率）、`duck_head_ma60_gap_pct`（鸭头顶距 60 线乖离）、`days_below_ma60_max`（连续破位天数）——以及 `old_duck_head_quality` 评分因子与 6 个形态硬过滤字段，用于排除五类假老鸭头：60 线下行诱多、回调放量出货、连续 3 日跌破 60 线、鸭嘴无量突破、高位诱多。鸭鼻孔采用「MA5/MA10 间隙越小越强」的贴合度判据，以兼容"死叉派"（必须死叉）与"粘合派"（不死叉、走平微拐）两派定义；关键风控使用乘性衰减而非线性扣分，避免总分溢出后被 `clip(0,100)` 上限掩盖、导致真假形态都拿满分。
- [修复] 分析任务异常时状态永久卡在 processing/97%（前端假死、无日志）：`src/services/task_queue.py` 的 `_execute_task` 异常捕获由 `except Exception` 升格为 `except BaseException`，并将 `logger.error` 改为 `logger.exception`。根因是 `asyncio.CancelledError`（Py3.8+ 属 `BaseException`，多来自 LLM/litellm 异步调用内部 task 被取消）穿透 `analyze_stock`/`process_single_stock`/`_execute_task` 三处 `except Exception` 冒泡，导致 worker 线程退出回归空闲但任务终态（COMPLETED/FAILED）未写入；与 SQLite/并发/写锁无关，单任务必现。修复后此类底层异常会让任务正确进入 FAILED（可重试）并记录完整 traceback，便于定位具体 LLM 调用。
- [新功能] 决策仪表盘微信(OpenClaw)推送失败自动轮询重试：首次推送失败（contextToken 失效/通道未恢复）时，登记已生成的决策仪表盘内容，由后台 daemon 线程每 30 分钟直接用原内容重推（不重新计算选股/分析），直到通道恢复（用户给 bot 发一条消息重建上下文后）推送成功；仅保留当天待重推，跨天旧内容自动丢弃。
- [改进] 决策仪表盘（含合并推送的个股+大盘复盘）仅在交易日推送：A 股周一至周五且交易所开盘才推送，周末与休市跳过；单股实时推送不受此限制。
- [新功能] 决策仪表盘微信超长自动分条：超过 OPENCLAW_WECHAT_MAX_BYTES(4000 字节)上限时，按 UTF-8 字节安全切片（不切断多字节字符）分多条发送，每条加 `[i/N]` 序号，不再截断；轮询重试同样分条，与首次一致。
- [修复] 微信(OpenClaw)发送链路自愈不再强删 context token：文本/图片推送失败重试与巡检均改用 `force_clear_token=False` 走 mtime/冷却护栏，仅删除早于最近失败时刻的过期 token，避免把临时 (a) iLink 掉线误判成 (b) contextToken 失效并主动制造 token 失效；`_repair_service` 默认参数同步由 `True` 改为 `False`，仅测试/明确场景传 `True`。
- [新功能] 实时放量预警标题增加涨跌图示：`Event Alert | 代码` 前根据实时行情追加 📈/📉/➡️，一眼区分上涨/下跌/平盘触发；当行情未直接返回 `change_pct` 时，用 `price/pre_close` 推导，兼容 pytdx 等数据源。
- [新功能] 报警标题增加股票名称：`Event Alert | 名称 代码`（如 `📈 Event Alert | 大族激光 000530.SZ`），通过 `DataFetcherManager.get_stock_name` 获取并进程内缓存。
- [新功能] OpenClaw 微信通道「到期前主动续期提醒」：在 iLink contextToken 临近预估有效期（默认 72h，提前 12h）时，趁仍可推送主动发微信提醒接收人「给 bot 发一条消息」以保持推送；发送失败则转网页弹窗兜底。可用 `OPENCLAW_CONTEXT_TTL_SECONDS` / `OPENCLAW_CONTEXT_REMIND_MARGIN_SECONDS` 微调（OpenClaw 不暴露真实过期时间，需按实际观察调整）。
- [新功能] 板块轮动当日刷新：盘中/收盘前 Tushare 历史数据还停留在上一交易日时，自动调用新浪实时板块行情（`stock_sector_spot`）补齐「今天」这一行（涨跌幅、总成交额、代表个股），板块资金流向实时兜底也返回当日日期；同时新浪接口无资金流数据，相关字段显示「暂缺」而非「+0.0亿」。兼容东财/新浪两套命名体系（通过 `_normalize_sector_name` 去除 `Ⅱ`/`Ⅲ` 等后缀）。
- [新功能] 实时放量预警增强：#6 信号分级（强/中/弱）、#8 行情来源与时效标注（如「通达信·延迟2s」）、#7 历史胜率提示（复用 DecisionSignalOutcomeService，样本≥20 时展示）、#11 非交易时段过滤（1 分钟 K 线 feed 仅在 INTRADAY/CLOSING_AUCTION 刷新，消除收盘后 pytdx「无法连接任何服务器」报错）。
- [改进] 实时行情主链路「通达信直连（pytdx）」被**硬置为绝对首位并彻底排除环境变量干扰**：`Config._resolve_realtime_source_priority()` 改为直接返回硬编码链 `pytdx,tencent,akshare_sina,efinance,akshare_em`，不再读取 `REALTIME_SOURCE_PRIORITY` 环境变量 / `.env`，也不再因 `TUSHARE_TOKEN` 存在而自动注入 tushare；`.env.example` 同步注明该变量已被忽略、后续降级顺序需改 `src/config.py`。目的是杜绝环境配置漂移把免费实时主链路降级。
- [新功能] 放量预警「斜率 / 峰值」接入 **1 分钟 K 线**：pytdx 主链路（A 股）在 `get_realtime_quote` 自算量比后，额外取 1 分钟 K 线（`get_intraday_bars`），用相邻分钟成交量增量换算成「当前这一分钟」的当量比写入 `RealtimeVolumeCache`（`record_1min_kline()`），使 `volume_spike_rt` 的斜率/峰值从 spot-quote（5 分钟采样）细化到分钟粒度，更贴合短线脉冲捕捉；该增强为 best-effort，失败不影响主流程，仅作用于 `RealtimeSource.PYTDX` + A 股。
- [新功能] OpenClaw 微信通道支持 **crash-loop breaker 自动自愈**：`OpenclawWechatSender` 新增 `_crash_loop_suppressed()` 探测（即便 gateway 端口仍可达，只要 `channels status` 账号 `lastError` 含 `crash loop breaker`/`unclean boot` 等关键字即判定通道被抑制），并在 `auto_heal` 探活中**强制重启 gateway** 清除 breaker（带 7 分钟冷却，避免过频反而被再次抑制）；`check_bot_health` 暴露 `crash_loop_suppressed`/`crash_loop_recovered` 字段，`main.py` 巡检任务在恢复时记 error 级日志。该状态不再被误判为需重扫或需用户发消息。
- [改进] **OpenClaw 接入配置持久化**：新增 `integrations/openclaw/` 作为 OpenClaw 接入配置的**唯一真源**（含 `workspace/AGENTS.md`、`skills/daily-stock-analysis/SKILL.md`、`agents/dsa_cmd.sh`），由 git 版本控制，彻底解决 `~/.openclaw/` 下这些文件被 gateway 更新/onboarding 覆盖后「命令信息丢失」的问题；新增 `scripts/sync_openclaw.sh` 一键把真源同步/恢复到 `~/.openclaw/`（幂等、只补不覆盖，保留 `hooks.token` 等密钥，自动把 `dsa-notify` agent、`hooks` 映射、`daily-stock-analysis` skill 段合并回 `openclaw.json`）。DSA 侧命令（`/analyze /market /menu` 等 11 条）本身硬编码于 `bot/commands/*.py`，由 `GET /api/v1/bot/commands` 实时生成，与 OpenClaw 无关、永不丢失。
- [修复] 放量预警「1 分钟预警盘中消失」回归：评估器 `evaluate_realtime_indicator_alert` 会先 `normalize_stock_code` 再查 `RealtimeVolumeCache.current_ratio`，但缓存写入（`record` / `record_1min_kline` / `record_many`）此前按**原始代码**（带 `.SH`/`.SZ`）建桶，导致 1 分钟 K 线喂入的数据（如 `600519.SH`）与评估器查找的归一化键（如 `600519`）对不上、`current_ratio` 永远返回 `None` →「量比数据不足」→ 预警静默失效。现 `RealtimeVolumeCache` 在全部读写入口统一用 `normalize_stock_code` 归一化 key（懒加载导入避免与 `data_provider.base` 循环依赖），spot-quote 与 1 分钟 K 线两条喂入路径写入与评估读取因此一致；`tests/test_volume_spike_rt_alert.py` 同步对齐「三路 OR 触发」语义（绝对量比≥阈值本身即触发，flat/declining 用例改用低于绝对阈值的数据以真实验证「无加速度不触发」路径）。
- [新功能] 选股结果列表新增「加入自选」按钮：每行最右侧新增自选列，未加入时显示「加入自选」（`PlusCircle`），已加入时显示「已加自选」（`CheckCircle2`）并禁用，点击调用 `useWatchlist.addToWatchlist(item.code)`，与首页自选列表使用同一套状态与 API。`StockScreeningPage.test.tsx` 新增两条对应用例（mock `useWatchlist` 以避免污染 `systemConfigApi`）。
- [修复] 横盘突破（`consolidation_breakout`）被错误路由到 `sideways_breakout` 专用形态识别器：8/27「策略归并」提交把原属 `sideways_breakout` 的专用路由条件改成了 `consolidation_breakout`，导致该策略绕过 `strategies/consolidation_breakout.yaml`（consolidation_quality 因子主导 + 行业分层采样 + LLM 排序），改用硬编码 snapshot 初筛与旧专用评分，表现为候选池从约 40 只暴涨到 800+ 只、分数封顶 100 分且无 LLM 排序。已移除该错误路由，策略回归通用 AlphaSift yaml 流程。
- [修复] 横盘突破（consolidation_breakout）评分改进：落地此前从未生效的「主升回落惩罚」——`consolidation_quality` 评分器新增按 `max_drawdown_20d_pct` 减分（回撤幅度超 8% 的部分 × 斜率 3.0，该字段为负值约定故取绝对值），用于压制"刚主升后又回落"的 V 形（此类形态此前易拿满分、挤占真正长期横盘票的位置）；`_compute_shape_features` 补 `ma_bullish` / `price_above_ma20` 两列，使已有的均线结构加分（5.0 / 3.0）真正生效——该加分逻辑此前因缺列被 `if "xxx" in df.columns` 守卫静默跳过。两处均作用于**通用 AlphaSift 管线**（该策略由 `strategies/consolidation_breakout.yaml` 驱动，配合上一条路由修复生效）。
- [新功能] 横盘突破（consolidation_breakout）选股结果默认自动加入分钟级放量/试盘预警：`screen()` 完成该策略选股后，为每只选出的股票按 (source=consolidation_breakout, alert_type=volume_spike_rt, target) 幂等注册 `volume_spike_rt` 实时量比规则（量比 ≥ 1.2x 或 5 分钟内出现 ≥ 1.3x 短爆即提醒），已存在则启用并更新参数、不存在则新建，避免每天重复堆积；注册异常被兜底，不影响选股返回。盘中放量（主力试盘）触发后由既有 `AlertWorker` 走全局通知通道（web / OpenClaw 微信）提醒，冷却与刷新周期对齐（5min），盘中持续提醒，不复用 24h 默认。
- [修复] 实时放量(volume_spike_rt)预警冷却由 24h 全局默认改为与刷新周期对齐的 5min：无显式 cooldown_policy 的实时类型规则走 REALTIME_ALERT_DEFAULT_COOLDOWN_SECONDS（=300s），盘中触发后可每刷新周期(~5min)再次提醒，不再一整天被压制；非实时类型仍保持 24h(DEFAULT_DB_ALERT_COOLDOWN_SECONDS)。
- [修复] 实时放量通知标题按来源加标签区分两类预警：横盘选股自动注册(source=consolidation_breakout)显示「横盘突破｜」、用户自选股显示「自选股｜」，正文不变；两类均走既有 web / OpenClaw 微信全局通道。
- [测试] 新增 alert_worker 实时放量短冷却与通知来源标签回归测试（tests/test_alert_worker_realtime_cooldown.py），覆盖实时/非实时冷却取值与标题标签。
- [改进] 选股 LLM 重排支持自动分批（map-reduce）：当候选池超过 20 只（或显式传入 `batch_size`，默认每批 15 只）时，`rank_candidates_with_metadata` 把候选切成多批独立 ranking，每批取 top 8 晋级决赛池，再对决赛池做一次完整 ranking 产出 `market_view`/`selection_logic`/`portfolio_risk` 等元数据；分批阶段使用紧凑 prompt 与 `rank_weight=1.0` 让 LLM 独立决定晋级名单、并随候选数动态放大 `max_tokens`，降低单 prompt token 压力与长列表 JSON 被截断（`empty_response`）的概率；任一单批失败时自动 fallback 到 screen_score 晋级、不影响其他批次与决赛。

- [新功能] 实时行情主链路接入「通达信直连」（pytdx）：`PytdxFetcher` 新增 `get_realtime_quote`（返回 `UnifiedRealtimeQuote`）与 `get_intraday_bars`（1 分钟 K 线，category=7），`RealtimeSource.PYTDX` 加入实时路由；默认 `REALTIME_SOURCE_PRIORITY` 将 `pytdx` 置于首位（A 股免费、无 Token、延迟更低），不可达时自动降级到腾讯/新浪/东财等 HTTP 源。实测 A 股实时行情延迟约 2–5s（直连握手），远优于 Tushare 实时接口；`DEFAULT_HOSTS` 首位补充了实测可用服务器。

- [新功能] 新增「网页弹窗」通知渠道（WEB）：无需任何用户配置，作为进程内 hub 兜底通道。后端 `Notification - Service` 将告警同时写入内存环形缓冲（`src/services/web_alert_hub.py`，容量 100、保留近 24h），新增 `GET /api/v1/alerts/web-popups?since=<id>` 拉取增量；前端 `useWebAlertPopups` 每 5 秒轮询、`WebAlertPopups` 右下角堆叠 toast 展示并 12s 自动关闭。需在 `NOTIFICATION_*_CHANNELS` 显式加入 `web` 才会路由。`detect_configured_channels`/`is_available()` 不计 WEB（避免污染「服务可用」信号），仅在路由显式包含 `web` 时投递；`ChannelDetector` 新增中文名「网页弹窗」，诊断 `CHANNEL_SPECS` 与 `CHANNEL_PROFILES` 均已覆盖。

- [改进] `volume_spike_rt` 新增「峰值」（peak）触发信号，解决短时脉冲放量漏报：之前仅靠「量比≥阈值」+「斜率≥阈值」，而采样间隔（30s）虽细，但 30 分钟斜率会把 3 分钟短时 spike 稀释掉、且绝对量比在评估时刻可能已回落。现第三路 OR 条件——窗口内出现过的最高量比 `peak >= min_peak_ratio` 即触发（`min_peak_ratio` 默认 0=关闭；存量 16 条规则已设为 1.5，即「窗口内任一样本达到放量阈值即提醒」）。`realtime_volume_cache` 新增 `peak_ratio()`。`params` 读取 `min_peak_ratio` 容错（缺省 0）。
- [备注] 采样间隔 `VOLUME_SPIKE_RT_REFRESH_INTERVAL_SECONDS` 实测已是 30s（足够细），无需改；`min_slope` 默认值改用 `params.get("min_slope", 0.0)` 容错。

- [改进] 量比徽章在 A 股非交易时段展示更精确的交易阶段标签：前端根据当前北京时间计算 A 股所处阶段（未开盘 / 午休 / 已收盘 / 非交易日），量比缺失时按阶段显示「未开盘」等字样，避免全天都显示「量比暂无」造成数据缺失的误解。新增 `getCurrentAshareMarketPhase()`、`getMarketFromStockCode()` 与 i18n 标签；更新 `VolumeSpikeBadge`、`HomeStockWorkspace`、`StockBarItem` 以透传市场信息。`07:51` 这类盘前场景会正确显示「未开盘」。盘中如仍出现「量比暂无」则是真实数据问题，需另查后端 enrich 链路
- [改进] 微信(OpenClaw)通道旁路巡检与发送链路自愈：检测到 iLink `prepare failed`（contextToken 失效）时，自动删除过期的 `context-tokens.json` 并重启 gateway（清 token 前记录失败时间戳，清 token 时校验文件 mtime 晚于失败时刻则跳过、并设 6h 冷却，避免误删用户刚重建的 token）；自愈后标记 `needs_user_message` 并提示用户给 bot 发一条消息重建对话上下文，不再盲目重试。巡检任务（`openclaw_wechat_health`，30min）开启 `auto_heal`：gateway 失活自动拉起、contextToken 失效自动重置并限频告警（contextToken 失效降级 warning、授权过期才 error）。修复 `check_bot_health(restart_gateway_if_down=True)` 调用未定义 `_maybe_restart_gateway()` 导致巡检任务每次崩溃的 latent bug
...
- [改进] 历史栏放量预警量比**不再自动定时刷新**（仅保留手动闪电按钮触发）：历史栏常 >50 只，与自选股叠加 5 分钟轮询会放大 422/请求压力。自选股仍按 9:00-15:00 每 5 分钟自动刷新；`HomePage` 移除历史栏自动 interval / timeout，`refreshHistorySpotQuotes` 仅由 `onRefreshHistorySpotQuotes` 手动调用
...
- [修复] 放量预警量比整批「量比暂无」另一根因：历史栏（常 >50 只）一次性请求触发后端 `WatchlistSpotQuotesRequest.codes` 的 `max_length=50` 校验返回 422，前端 `fetchWatchlistSpotQuotes` 的 catch 静默吞掉并以空 Map 覆盖，导致整栏量比显示为「暂无」。修复：后端 `codes` 上限放宽到 300；前端改为按 40 只一批分批调用（避开 422 与 axios 30s 超时）并把 catch 改为向上抛错，由调用方 `refreshWatchlistSpotQuotes`/`refreshHistorySpotQuotes` 保留上次成功数据而非清空成「暂无」；新增 2 项 vitest 回归测试覆盖"请求失败抛错"与"超长列表分批调用"
- [改进] 选股页「昨日」Tab 改为**点开即自动加载**：点击 Tab 时无需先点「一键回测」，自动读取本地选股缓存（`dsa.alphasift.screenCache.v2`）并调用 `backtestYesterday` 计算昨日标的今日表现；Tab 内展示全部候选（含加载态/空态/重新计算按钮），涨停标的行以红色流光（`animate-pulse-glow`）特效高亮、涨≥5% 行以绿光高亮，保留涨停/涨≥5% 状态徽标与涨幅排序
- [修复] 选股页切换策略、市场、返回数量或重新选股时，昨日复盘结果立即被清空并随新结果自动重算，避免底部「昨日」面板展示过期数据；新增 `yesterdayRequestIdRef` 护栏，切换/重算时旧请求结果不会覆盖新结果
- [修复] 选股页「昨日」Tab 语义修正：自动加载时 `backtestYesterday` 的 payload `price` 改为 `null`，让后端以昨天收盘价为信号基准（而非当前实时价），避免"信号价≈今收"的反直觉显示；面板标题改为「昨日选出的票 · 今日表现」并加注「信号日=昨天、信号价=昨收、涨幅=今天实际表现」，明确这是昨天缓存选出的票今天的涨幅
...
- [修复] 微信(OpenClaw)推送频繁超时失败：`openclaw` CLI（Node.js）冷启动实测约 30–40s，图片推送（CDN 上传）更长，原 60s 最小 CLI 超时（`_OPENCLAW_CLI_MIN_TIMEOUT_SECONDS`）经常不足，导致放量预警等实时通知被误判为超时。将最小超时提升到 120s，避免长大图片或冷启动场景下的假超时
- [修复] `volume_spike_rt` 实时放量预警在午休/盘前/盘后/非交易日误触发：预警入口 `evaluate_realtime_indicator_alert` 原先只基于缓存的盘前数据判断，午休期间市场已收盘仍会推送。新增市场时段护栏——先规范化股票代码解析市场，再用 `infer_market_phase` 识别阶段，仅在 `INTRADAY`/`CLOSING_AUCTION`（或日历不可用的 `UNKNOWN`，fail-open）才评估；`PREMARKET`/`LUNCH_BREAK`/`POSTMARKET`/`NON_TRADING` 直接跳过并返回「市场未开盘，跳过实时指标检查」。注意：必须先 `normalize_stock_code` 再 `get_market_for_stock`，否则带后缀代码（如 `000966.SZ`）无法识别市场导致护栏失效
- [改进] `volume_spike_rt` 放量预警改为「量比≥阈值 **或** 斜率≥阈值」双信号 OR 口径（原实现是量比硬门槛，斜率不参与触发，导致「正在急速放量但绝对值未到 1.5」的票漏报，如 600403 大有能源）。判定逻辑：当 `current >= min_ratio` **或**（`slope_per_5min >= min_slope` 且 `current > 1.0` 防噪音）时触发。已将全部 16 条存量 `volume_spike_rt` 规则的 `min_slope` 从 `0.0`（原语义＝关闭斜率检查）批量更新为 `0.15`（per-5min）；`min_slope=0.0` 仍保留「仅看病价值」语义向后兼容
...
- [新功能] 新增 `golden_pit` 黄金坑选股策略（参考一鸣食品 2026-07-22 → 2026-08-11 横盘 → 拉伸 → 黄金坑洗盘 → 再启动模式），侧重 20 日内有 4-12% 回撤、价格守在 MA20 上方、当日微红带量等再启动早期信号，硬约束涵盖涨幅/振幅/量比/趋势结构，因子权重 momentum 0.30 / reversal 0.22 / activity 0.15 / theme_heat 0.13 / stability 0.10；同步增强 `_prepare_alphasift_runtime_env` 在启动时把 `src/services/screening/strategies/` 下的 DSA 自有策略文件按 mtime 增量拷到 alphasift 打包目录，避免 reinstall 后丢失，源仍以 DSA 为准（strategies 文件位置保持单一真源）。
- [修复] alphasift 0.2.0 strict loader 拒绝含 AI-agent enrichment 字段（`analysis_skills`、`aliases`、`instructions`、`market_regimes` 等）的策略 YAML，导致 7 份原 alphasift ship 策略 + DSA 黄金坑只能加载 6 份。同时 `consolidation_breakout.yaml` 因仅在 alphasift 打包目录、DSA 未镜像而缺失。修复：`src/services/screening_service.py::_build_alphasift_clean_strategies_dir` 在启动时构建 `src/services/screening/.alpha_clean_strategies/`，按 alphasift `_TOP_LEVEL_KEYS / _SCREENING_KEYS / _SCORING_PROFILE_KEYS / _RISK_PROFILE_KEYS / _PORTFOLIO_PROFILE_KEYS / _SCORECARD_PROFILE_KEYS / _EVENT_PROFILE_KEYS / _STYLE_KEYS / _HARD_FILTER_KEYS` 多层递归剥除 unknown keys（保留 DSA 源里的 agent 字段），并把 DSA 没有但 alphasift 打包有的策略（如 `consolidation_breakout.yaml`）一并镜像。DSA 仍是单一 source of truth。`_STRATEGY_DIR_CACHE` 同步对齐以避免误命 cache。修复后 `/screening` 端点稳定返回 13 份策略。
- [修复] `golden_pit` 黄金坑 hard filters 第一次切片过于严苛（`change_60d_min: 5.0`、`pullback_to_ma20_pct: ±5`、`require_ma_bullish: true` 等），比仓库其他策略严了一个数量级，导致 100 个日 K 富集候选被逐条削到 0，下游 LLM 排序无候选可排。已对齐仓库策略的放宽惯例，并按"早期再启动窗口"重新校准：`change_60d_min: -10`、`change_pct_min: -3.5`、`change_20d_min: 3.0`、`max_drawdown_20d_pct: 3-18`、`pullback_to_ma20_pct: -10..10`、`require_ma_bullish: false`、`require_price_above_ma20: false`、`signal_score_min: 30`、`macd_status_whitelist` 含 bearish；scoring profile 同步把 `reversal_distance_penalty_slope` 调到 8 让 `reversal_ideal_change_pct=-4` 真的扣动排序，让真正处于黄金坑+早期再启动的标的能进 top-5。
- [修复] `golden_pit` 黄金坑 v3 仍有部分 filter 会因 daily K-line 富集为 NaN 而被 alphasift `_filter_min` 的 `series.notna()` 隐式淘汰（哪怕阈值很宽松，如 `change_60d_min=-10` 仍砍掉 45/100）。v4：彻底移除所有 daily-feature 类硬过滤（`change_*_d`、`pullback_to_ma20_pct`、`max_drawdown_20d_pct`、`breakout_20d_pct_*`、`volume_ratio_20d_*`、`range_20d_pct_*`、`body_pct_*`、`signal_score_min`、`macd_status_whitelist`、`rsi_status_whitelist`、`consolidation_days_*`、`volatility_*`、`atr_*`），让 `requires_daily_features` 返回 False，整个 daily 富集阶段不再参与。仅保留纯 snapshot 级过滤（`exclude_st`、`price 3..300`、`amount_min=5e7`、`market_cap_min=2e9`、`change_pct_min=-3.5`）；scoring_profile 仍按 momentum 0.30 / reversal 0.22 / activity 0.15 排序，`reversal_ideal_change_pct=-4` + `reversal_distance_penalty_slope=8` 真正承担"早期再启动"信号锚定。同步清理 `data/alphasift/screencache/golden_pit.json` 强制刷新。
- [修复] v4 退化为"动量 + 当日不暴跌"通用选股（中孚实业 / 九安医疗 / 招商南油 / 南山铝业 均非黄金坑形态）。v5 改用仓库标准 `bottom_accumulation.yaml` 的 `-999 / 9999` 禁用写法与"主力信号 via custom factor"的模式：
  - hard_filters 显式启用 `max_drawdown_20d_pct: 3-30`（必有"坑"）+ `pullback_to_ma20_pct: -15..15`（MA20 附近再启动窗口）+ `change_pct: -4..9`（避免单日大跌）+ 基础流动性 / 价位 / ST。
  - 所有其它 NaN-prone / 不适用的 daily filter 写 -999 / 9999 / `[]` 关闭，避开了 `_filter_min` 对 NaN 的隐性淘汰。
  - factor_weights 主权重移到 `consolidation_quality: 0.32`（对应横盘后蓄势）+ `bottom_accumulation_quality: 0.18`（底吸筹特征）+ `capital_heat_quality: 0.16`（资金关注度）+ `momentum: 0.15`、`reversal: 0.10`、`stability: 0.05`、`liquidity: 0.04`。这样 scoring 会优先把"近期经历 3-30% 拉回 + 价格在 MA20 附近 + 有横盘整理 + 资金关注"的票顶上来。
  - 同步清 `data/alphasift/screencache/golden_pit.json`、`_STRATEGY_DIR_CACHE` 强制重新计算。- [新功能] 历史报告 Markdown 和分享海报同步展示"主力资金"摘要：`_generate_single_stock_markdown` 增加资金 section；`src/share_image.py` 个股海报增加主力资金 block。
