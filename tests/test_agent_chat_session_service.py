...
def _isolate_real_env(monkeypatch, tmp_path):
    """Isolate tests from the real .env so DatabaseManager/get_config()
    cannot leak real runtime keys (e.g. LITELLM_FALLBACK_MODELS) into
    os.environ and pollute other test modules that rely on a temp env."""
